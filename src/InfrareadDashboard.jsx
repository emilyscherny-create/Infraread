 ...
 *
 * This ensures the buttons are always visible and the analysis tool is usable
 * either via the parent-provided runAnalysis or via the component's own analyzer.
 */
...
function downloadJSON(data, filename = "infraread-session.json") {
  try {
    ...
    // (In the app runtime this will work fine)
  }
}
...
export default function InfrareadDashboard(props) {
  ...
  function handleRunAnalysis() {
    if (typeof runAnalysisProp === "function") {
      // If parent runAnalysis returns something, capture it in parent or local analysis
      const maybe = runAnalysisProp();
      // If parent didn't set analysis via setAnalysis, also compute and set locally for UI
      if (!setAnalysisProp) {
        // allow parent runAnalysis a tick to update; otherwise compute locally
        setTimeout(() => {
          internalRunAnalysis();
        }, 50);
      }
  ...
  // helper: simulate a small session so UI and buttons demonstrate functionality
  function simulateTypingSample() {
    ...
    // compute analysis immediately for convenience
    setTimeout(() => {
    ...
    <aside className="infraread-dashboard" style={containerStyle}>
      ...
      <div style={boxStyle}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 8 }}>
          <button
            style={{ padding: "8px 10px", borderRadius: 6, background: "#10b981", color: "#fff", border: "none", cursor: "pointer" }}
          ...
          <button
            style={{ padding: "8px 10px", borderRadius: 6, background: "#0ea5e9", color: "#fff", border: "none", cursor: "pointer" }}
          ...
          <button
            style={{ padding: "8px 10px", borderRadius: 6, background: "#06b6d4", color: "#fff", border: "none", cursor: "pointer" }}
          ...
          <button
            style={{ padding: "8px 10px", borderRadius: 6, background: "#6366f1", color: "#fff", border: "none", cursor: "pointer" }}
          ...
          <button
            style={{ padding: "8px 10px", borderRadius: 6, background: "#e2e8f0", color: "#0f172a", border: "none", cursor: "pointer" }}
